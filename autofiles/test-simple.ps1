# Simple Test Script for DocGO Local Environment
Write-Host "Testing DocGO Local Environment..." -ForegroundColor Green
Write-Host "===================================" -ForegroundColor Green

# Test Infrastructure Services
Write-Host "`nTesting Infrastructure Services..." -ForegroundColor Yellow

# Test MariaDB
try {
    $response = Invoke-WebRequest -Uri "http://localhost:3307" -TimeoutSec 5 -ErrorAction Stop
    Write-Host "MariaDB (Port 3307): Running" -ForegroundColor Green
} catch {
    Write-Host "MariaDB (Port 3307): Not accessible" -ForegroundColor Red
}
...
Write-Host "Microservices: 5 services with volume mount" -ForegroundColor White
Write-Host "API Documentation: /docs endpoints" -ForegroundColor White
Write-Host "Hot Reload: Code changes will be reflected immediately" -ForegroundColor White

Write-Host "`nDocGO Local Environment is ready!" -ForegroundColor Green
Write-Host "You can start testing the upload file flow:" -ForegroundColor Yellow
