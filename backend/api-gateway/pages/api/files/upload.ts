...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Append repository_id if provided by frontend
    const repositoryId = Array.isArray(fields.repository_id) ? fields.repository_id[0] : fields.repository_id
    if (repositoryId) {
    ...
    const automationBase = process.env.AUTOMATION_SERVICE_URL || 'http://automation-service:8003'
    let automationUrl: URL
    try {
      automationUrl = new URL(`${automationBase}/api/v1/automation-service/files`)
    } catch (e) {
      console.error('Invalid AUTOMATION_SERVICE_URL:', process.env.AUTOMATION_SERVICE_URL)
      return res.status(500).json({ 
        error: 'Internal server error', 
        message: 'Invalid automation service URL configuration' 
      })
    }
