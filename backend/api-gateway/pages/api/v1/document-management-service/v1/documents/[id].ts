...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const fullPath = `/api/v1/document-management-service/v1/documents/${id}`;
  
  logger.info(`🔄 Documents Detail API Proxy: ${method} ${fullPath}`);
  ...
  try {
    ...
    Object.keys(req.query).forEach((key) => {
      if (key !== 'id') { // Exclude the id parameter
    ...
    logger.info(`✅ Documents Detail API Response: ${response.status}`);
    return res.status(response.status).json(response.data);

  } catch (error: any) {
    logger.error('❌ Documents Detail API Error:', error);
    return res.status(503).json({
      error: 'Service unavailable',
      message: 'Document Management Service is not available'
    });
