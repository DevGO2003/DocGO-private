...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const backendUrl = `${USER_MANAGEMENT_SERVICE_URL}/api/v1/user-management-service/organizations/${id}/members/${userId}`
    
    console.log(`[Organizations Member Proxy] ${req.method} ${backendUrl}`)
    ...
    console.error('[Organizations Member Proxy] Error:', error)
    ...
    return res.status(500).json({
      ...
      requestId,
      path: req.url || `/api/v1/organizations/${id}/members/${userId}`
    })
  }
}
