...
```powershell
cd "D:\nam cuoi\New folder\backend\user-service"
...
Sửa `.env` để dùng MySQL và DB `docgo_user_service` (ví dụ user root/password `sapassword`):
```env
DATABASE_URL=mysql+pymysql://root:sapassword@127.0.0.1:3306/docgo_user_service
...
- **Xem theo ID**
```bash
curl "http://localhost:8000/api/v1/users/1?system_id=system1"
```
- **Xem theo email**
```bash
curl "http://localhost:8000/api/v1/users/email/john.doe@example.com?system_id=system1"
```
- **Sửa user**
```bash
curl -X PUT "http://localhost:8000/api/v1/users/1?system_id=system1" \
  -H "Content-Type: application/json" \
  -d '{"full_name":"Johnathan Doe"}'
```
- **Xóa user**
```bash
curl -X DELETE "http://localhost:8000/api/v1/users/1?system_id=system1"
```
- **Tìm kiếm (phân trang)**
```bash
curl "http://localhost:8000/api/v1/users?system_id=system1&full_name=john&page=1&size=10"
```
...
- **Xóa avatar**
```bash
curl -X DELETE "http://localhost:8000/api/v1/users/1/avatar?system_id=system1"
```
- **User kèm trạng thái duyệt**
```bash
curl "http://localhost:8000/api/v1/users/1/with-approval?system_id=system1"
```
- **Lấy roles từ Identity Service**
```bash
curl "http://localhost:8000/api/v1/users/1/roles"
```
- **Danh sách users kèm approvals theo system**
```bash
curl "http://localhost:8000/api/v1/users/system/system1/with-approvals"
```
...
```bash
curl "http://localhost:8000/api/v1/approvals/1?system_id=system1"
```
- **Xem theo user_id**
```bash
curl "http://localhost:8000/api/v1/approvals/user/1?system_id=system1"
```
- **Cập nhật trạng thái** (yêu cầu `approver_id`)
```bash
curl -X PUT "http://localhost:8000/api/v1/approvals/1?system_id=system1" \
  -H "Content-Type: application/json" \
  -d '{"status":"approved","approver_id":100,"notes":"OK"}'
```
- **Phím tắt Approve/Reject**
```bash
curl -X POST "http://localhost:8000/api/v1/approvals/1/approve?system_id=system1&approver_id=100"
curl -X POST "http://localhost:8000/api/v1/approvals/1/reject?system_id=system1&approver_id=100&notes=Insufficient+docs"
```
- **Tìm kiếm**
```bash
curl "http://localhost:8000/api/v1/approvals?system_id=system1&status=pending&page=1&size=10"
```
- **Bulk update**
```bash
curl -X POST "http://localhost:8000/api/v1/approvals/bulk" \
  -H "Content-Type: application/json" \
  -d '{
    "user_ids":[1,2,3],
    "system_id":"system1",
...
- **Thống kê**
```bash
curl "http://localhost:8000/api/v1/approvals/statistics/system1"
```
- **Danh sách pending/approved**
```bash
curl "http://localhost:8000/api/v1/approvals/pending/system1?page=1&size=10"
curl "http://localhost:8000/api/v1/approvals/approved/system1?page=1&size=10"
```
...
- **Endpoint**: `POST /api/v1/user-service/approvals/`
...
### 3) Phê duyệt/Từ chối
- **Endpoint chính**: `PUT /api/v1/user-service/approvals/{approval_id}?system_id=...`
- **Shortcut**:
  - Approve: `POST /api/v1/user-service/approvals/{approval_id}/approve?system_id=...&approver_id=...`
  - Reject: `POST /api/v1/user-service/approvals/{approval_id}/reject?system_id=...&approver_id=...&notes=...`
- **Hành vi**:
...
- Lấy role từ Identity Service:
  - **Endpoint**: `GET /api/v1/user-service/users/{user_id}/roles`
...
### 5) Quản lý avatar (S3)
- **Upload**: `POST /api/v1/user-service/users/{user_id}/avatar` (multipart: `system_id`, `file`)
...
- **Xóa avatar**: `DELETE /api/v1/user-service/users/{user_id}/avatar?system_id=...`
  - Xóa file trên S3 (nếu có) và cập nhật hồ sơ.
- Khi xóa user (mục 7), avatar sẽ được xóa kèm.

### 6) Tra cứu và thống kê
- **Users (search)**: `GET /api/v1/user-service/users?system_id=...&full_name=...&email=...&page=1&size=10`
- **User + Approval**: `GET /api/v1/user-service/users/{user_id}/with-approval?system_id=...`
- **Approvals (search)**: `GET /api/v1/user-service/approvals?system_id=...&status=...&page=...&size=...`
- **Thống kê**: `GET /api/v1/user-service/approvals/statistics/{system_id}`
- **Danh sách theo trạng thái**:
  - Pending: `GET /api/v1/user-service/approvals/pending/{system_id}`
  - Approved: `GET /api/v1/user-service/approvals/approved/{system_id}`

### 7) Xóa người dùng (Cascade xử lý dữ liệu liên quan)
- **Endpoint**: `DELETE /api/v1/user-service/users/{user_id}?system_id=...`
- **Hành vi**:
