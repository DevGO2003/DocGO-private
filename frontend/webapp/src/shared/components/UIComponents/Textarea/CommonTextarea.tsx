...
export const CommonTextarea = forwardRef<HTMLTextAreaElement, CommonTextareaProps>(
  ({ className, label, error, helperText, value, resizable = true, ...props }, ref) => {
    ...
    const drawCanvas = () => {
      if (canvasRef.current && containerRef.current) {
        ...
        // Draw hand-drawn textarea border
      ...
      <CommonFont className="w-full space-y-2">
        ...
        >
          ...
          <textarea
            ref={ref}
            value={value}
            className={`relative w-full px-4 py-3 bg-transparent focus:outline-none resize-${resizable ? 'vertical' : 'none'} ${className || ''}`}
            {...props}
          />
        </div>
        {error && (
