...
/**
 * API Client
 * Centralized HTTP client với authentication, error handling, và token refresh
 */
class ApiClient {
  ...
  async postWithProgress<T>(url: string, data?: any, config?: AxiosRequestConfig & { onUploadProgress?: (progressEvent: any) => void }): Promise<AxiosResponse<ApiResponse<T>>> {
    return this.client.post(url, data, config)
  }
