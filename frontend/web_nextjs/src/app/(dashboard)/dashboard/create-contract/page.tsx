...
import { mapContractTypeToEnum } from '@/utils/contractTypeMapping'
import OCRUpload from '@/components/OCRUpload'

export default function CreateContractPage() {
  // Tab state
  const [activeTab, setActiveTab] = useState<'ocr' | 'upload' | 'manual'>('ocr')
  ...
  const handleOcrResult = (result: any) => {
    setOcrResult(result)
    // Auto-fill form with OCR result
    if (result) {
      if (result.title) setTitle(result.title)
      if (result.contractType) setContractType(result.contractType)
      if (result.object) setObject(result.object)
      if (result.effectiveDate) setEffectiveDate(result.effectiveDate)
      if (result.term) setTerm(result.term)
      if (result.content) setContent(result.content)
      if (result.parties) setParties(JSON.stringify(result.parties, null, 2))
      if (result.keyClauses) setKeyClauses(JSON.stringify(result.keyClauses, null, 2))
      if (result.paymentDetails) {
        if (result.paymentDetails.totalValue) setPaymentTotalValue(result.paymentDetails.totalValue.toString())
        if (result.paymentDetails.currency) setPaymentCurrency(result.paymentDetails.currency)
        if (result.paymentDetails.schedule) setPaymentSchedule(result.paymentDetails.schedule)
        if (result.paymentDetails.paymentMethod) setPaymentMethod(result.paymentDetails.paymentMethod)
      }
    }
  }
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      ...
      try {
        // Try file storage service first
        const formData = new FormData()
        formData.append('file', uploadedFile)

        const res = await fetch('/api/v1/file-storage-asset-service/upload', {
          method: 'POST',
          body: formData
        })
        ...
        // Fallback: Just simulate successful upload
        console.log('File storage service not available, simulating upload')
      }
      
      // Auto-fill basic info from filename
      if (uploadedFile.name) {
        const fileName = uploadedFile.name.replace(/\.[^/.]+$/, '') // Remove extension
        setTitle(fileName)
        setContent(`Tài liệu đã upload: ${uploadedFile.name}`)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mappedContractType = contractType.trim() 
        ? mapContractTypeToEnum(contractType.trim())
        : 'EMPLOYMENT_CONTRACT'
      ...
      const payload: any = {
        ...
        contractType: mappedContractType, // Use mapped enum value
      ...
      // Optional basic fields - contractType already mapped above
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-r from-teal-500 to-green-600 rounded-3xl shadow-2xl transform rotate-1"></div>
              <div className="relative bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-gray-600 text-lg">Sử dụng OCR để trích xuất thông tin từ file hoặc văn bản.</p>
                  </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                          Hỗ trợ tất cả loại file (PDF, DOCX, TXT, JPG, PNG, etc.)
                        </p>
                      </div>
                      <input
                        type="file"
                        ...
                        className="hidden"
                        id="file-upload-normal"
                        accept="*/*"
                      />
                      <label
                        htmlFor="file-upload-normal"
                  ...
                  {uploadError && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h3 className="text-sm font-semibold text-red-800">Lỗi upload</h3>
                ...
                <div className="grid gap-8 md:grid-cols-2 xl:grid-cols-2">
                  ...
                  <div className="grid gap-4">
                    ...
                    <input
                      ...
                      placeholder="VD: SERVICE_AGREEMENT, PURCHASE_AGREEMENT, PARTNERSHIP_AGREEMENT"
                      className="border border-gray-200 rounded-xl px-6 py-4 text-base focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md"
                    />
                  </div>
                  <div className="grid gap-4">
          ...
          {ocrResult && (
            <div className="mt-6 bg-green-50 border border-green-200 rounded-2xl p-6">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="text-lg font-semibold text-green-800">Kết quả OCR</h3>
              </div>
          ...
          {ocrError && (
            <div className="mt-6 bg-red-50 border border-red-200 rounded-2xl p-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-red-800">Lỗi OCR</h3>
