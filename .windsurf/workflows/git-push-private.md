---
description: Git Push Private Command
---
...
## Lệnh dưới đây sẽ bổ sung vào Windsurf TODO, lưu ý Phần nào trước thì phải thực thiện xong trước rồi mới qua Phần tiếp theo, ko được làm song song các Phần:
  ...
  2) <private>: đọc REMOTE_PRIVATE từ ".windsurf/tools/github/env/.env"
  ...
  6) <restore>: file trong đường dẫn ".windsurf\workflows\git-restore-env-local.md"
  7) <backup>: file trong đường dẫn ".windsurf\workflows\git-backup-env.md"
  8) <mcp-discord>: file trong đường dẫn ".windsurf\workflows\send-discord-message.md"
  9) <push-origin>: file trong đường dẫn ".windsurf\workflows\git-push.md"
  ...
  6) Khi push các file nhạy cảm (.env*, token, ...), nếu các file này đã bị .gitignore thì cần dùng `git add -f <file>` (hoặc `git add --force <file>`) để ép add vào stage, hoặc đảm bảo .gitignore không chặn các file này.
     ...
     - Nếu gặp conflict phức tạp hoặc nhiều file, lập Windsurf TODO ghi lại các file/nội dung conflict để giải quyết dần.
