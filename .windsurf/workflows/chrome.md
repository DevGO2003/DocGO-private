---
description: Chrome Command - Điều khiển Chrome qua MCP Chrome DevTools
---
...
## Yêu cầu
- Đã cấu hình `chrome-devtools` trong `.windsurf/mcp.json` với `npx chrome-devtools-mcp@latest`
- Nếu vừa chỉnh `.windsurf/mcp.json`, hãy restart Windsurf để nạp MCP server
...
- Khi gặp SSL/self-signed, cân nhắc bật `--acceptInsecureCerts` trong `.windsurf/mcp.json`

## Troubleshooting
- "Chrome MCP chưa sẵn sàng": Restart Windsurf để nạp MCP
